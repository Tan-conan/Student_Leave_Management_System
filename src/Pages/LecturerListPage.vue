...
async function fetchLecturersList() {
  try {
    ...
    const res = await axios.post('http://localhost:3000/api/lecturerManage/fetchLecturersList', {
      programID: userProgramId.value,
    })
...
async function approveLecturer() {
  ...
  try {
    const res = await axios.post('http://localhost:3000/api/lecturerManage/approveLecturer', {
...
async function rejectLecturer() {
  await sessionChecker();
  try {
    const res = await axios.post('http://localhost:3000/api/lecturerManage/deleteLecturer', {
      lecturer_id: selectedLecturerRow.value.lecturer_id,
    })
